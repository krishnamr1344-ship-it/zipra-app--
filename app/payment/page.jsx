...
import { ChevronLeft, CreditCard, Wallet, Banknote, Check, Lock, ShieldCheck } from "lucide-react";
...
export default function PaymentPage() {
  ...
  const pay = async () => {
    ...
    try {
      if (payment === "razorpay") {
        await paymentsService.createRazorpay(total);
        await paymentsService.process({ order_id: orderId, method: "razorpay" });
      } else if (payment === "cod") {
        await paymentsService.process({ order_id: orderId, method: "cod" });
      } else {
        await paymentsService.process({ order_id: orderId, method: "wallet" });
      }
      await clearCart();
      setPaying(false);
      ...
      setPaying(false);
      const msg = err?.response?.data?.detail || "Payment could not be completed";
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-border bg-card/95 pb-[env(safe-area-inset-bottom)] backdrop-blur lg:static lg:border-0 lg:bg-transparent">
        <div className="container-app py-3">
          <Button fullWidth variant="gradient" size="lg" disabled={paying} onClick={pay} className="gap-2">
            <Lock className="h-4 w-4" /> {paying ? "Processing…" : `Pay ${formatPrice(total)}`}
          </Button>
        </div>
      </div>
    </div>
